...
class SlidesController extends Controller{
			...
			if($this->Slide->validates($this->request->data)){
				if(!empty($_FILES['file']['name'])){
					if(strpos($_FILES['file']['type'], 'image') !== false){
						...
						$dirFr = WEBROOT.DS.'slideshow'.DS.'images';
						$dirFrtmb = WEBROOT.DS.'slideshow'.DS.'images'.DS.'tmb';
						...
						if ($ExtensionPresumee == 'jpg' || $ExtensionPresumee == 'jpeg' || $ExtensionPresumee == 'pjpg' || $ExtensionPresumee == 'pjpeg' || $ExtensionPresumee == 'gif' || $ExtensionPresumee == 'png')
						{
							...
							if($ImageSlide['mime'] == $ListeExtension[$ExtensionPresumee]  || $ImageSlide['mime'] == $ListeExtensionIE[$ExtensionPresumee])
							{
								...
								switch ( $ExtensionPresumee ) {
									...
									imagejpeg($NouvelleImage , WEBROOT.DS.'slideshow'.DS.'images'.DS.'tmb/'.$id.'_'.$NomImageExploitable.'.'.$ExtensionPresumee, 100);
									...
									imagejpeg($ImageChoisie ,  WEBROOT.DS.'slideshow'.DS.'images'.DS.$id.'_'.$NomImageExploitable.'_FullImage.'.$ExtensionPresumee, 100);
									...
									imagegif($NouvelleImage , WEBROOT.DS.'slideshow'.DS.'images'.DS.'tmb/'.$id.'_'.$NomImageExploitable.'.'.$ExtensionPresumee);

									// Pour enregistrer au format gif [redimensionée] GIF FR
									imagegif($NouvelleImage , WEBROOT.DS.'slideshow'.DS.'images'.DS.'tmb/'.$id.'_'.$NomImageExploitable.'.'.$ExtensionPresumee);

									//Enregistrement de l'image aux dimensions d'origine : GIF 
									imagegif($ImageChoisie ,  WEBROOT.DS.'slideshow'.DS.'images'.DS.$id.'_'.$NomImageExploitable.'_FullImage.'.$ExtensionPresumee);
									...
									imagepng($NouvelleImage , WEBROOT.DS.'slideshow'.DS.'images'.DS.'tmb/'.$id.'_'.$NomImageExploitable.'.'.$ExtensionPresumee, 9); // qualité de 0 à 9 pour les PNG

									// Pour enregistrer au format gif [redimensionée] PNG FR
									imagepng($NouvelleImage , WEBROOT.DS.'slideshow'.DS.'images'.DS.'tmb/'.$id.'_'.$NomImageExploitable.'.'.$ExtensionPresumee, 9); // qualité de 0 à 9 pour les PNG

									//Enregistrement de l'image aux dimensions d'origine : PNG NL
									imagepng($ImageChoisie ,  WEBROOT.DS.'slideshow'.DS.'images'.DS.$id.'_'.$NomImageExploitable.'_FullImage.'.$ExtensionPresumee, 9); // qualité de 0 à 9 pour les PNG
	...
	**/
	function admin_delete($id){
		...
		if(!empty($slide->file)){ unlink(WEBROOT.DS.'slideshow'.DS.'images'.DS.$slide->file);} // NL
		//Suppression des fichiers thumbs
		if(!empty($slide->file)){ unlink(WEBROOT.DS.'slideshow'.DS.'images'.DS.$slide->file_tmb);} // FR
		if(!empty($slide->file)){ unlink(WEBROOT.DS.'slideshow'.DS.'images'.DS.$slide->file_tmb);} // NL		
		}
